...
void CWebserverRequest::SplitParameter(char *param_copy)
{
	...
	if (p == NULL) {
		char number_buf[20];
		sprintf(number_buf, "%d", (int)(ParameterList.size()+1));
...
//		printf("%s\n", rawbuffer.substr(0, ende - 1).c_str());
...
bool CWebserverRequest::HandleUpload()
{
	...
	if(HeaderList["Content-Length"] != "")
	{
		...
		if(!buffer2)
		{
			aprintf("Kein Speicher f¸r upload\n");
			return false;
		}
		long long gelesen = 0;
...
{
	if(( URL.length() == 1) && (URL[URL.length()-1] == '/' ))		// Wenn letztes Zeichen ein / ist dann index.html anh‰ngen
	{
		Path = URL;
		Filename = "index.html";
	}
...
{
	bool ret = Parent->SendFile(this, filename);
...
std::string CWebserverRequest::GetFileName(std::string path, std::string filename)
{
	...
	if( access(std::string(Parent->cfg.PublicDocumentRoot + tmpfilename).c_str(),4) == 0)
			tmpfilename = Parent->cfg.PublicDocumentRoot + tmpfilename;
	else if(access(std::string(Parent->cfg.PrivateDocumentRoot + tmpfilename).c_str(),4) == 0)
			tmpfilename = Parent->cfg.PrivateDocumentRoot + tmpfilename;
...
size_t CWebserverRequest::ParseBuffer(char *file_buffer, long file_length, char *out_buffer, long out_buffer_size, CStringList &params)
{
